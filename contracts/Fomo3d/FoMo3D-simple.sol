...
contract FoMo3Dlong is modularLong {
    ...
    {  
        ...
        if (_now > round_[_rID].strt + rndGap_ && (_now <= round_[_rID].end || (_now > round_[_rID].end && round_[_rID].plyr == 0)))
            ...
            return ( 000 075 000 000 000 000 ); // init
    }
...
library F3DKeysCalcLong {
    ...
    function keys(uint256 _eth) 
        ...
        returns(uint256)
    {
        return ((((((_eth).mul(1 ether)).mul(312500000 ether)).add(562498828125610351562500000000000000000000000 ether)).sqrt()).sub(74999921875000000000000000000000)) / (156250000);
    }
